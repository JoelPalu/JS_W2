...
console.log(restaurants.forEach((restaurant) => console.log(restaurant.location.coordinates)));
...
function calculateDistance(userLocation, restaurant) {
  const x = restaurant.coordinates[0] - userLocation.y;
  const y = restaurant.coordinates[1] - userLocation.x;
  return Math.sqrt(x * x + y * y);
}

function render(userLocation) {
  restaurants.forEach((restaurant) => {
    restaurant.distance = calculateDistance(userLocation, restaurant.location);
  });
  restaurants.sort((a, b) => a.distance - b.distance);

  restaurants.forEach((restaurant) => {
    console.log(restaurant.name, restaurant.distance);
  });
  ...
  restaurants.forEach((restaurant) => {
    ...
    tr.appendChild(td2);
    td3.textContent = restaurant.distance;
    console.log(restaurant.distance);
    tr.appendChild(td3);
    result.insertAdjacentHTML('beforeend', tr.outerHTML);
  });
